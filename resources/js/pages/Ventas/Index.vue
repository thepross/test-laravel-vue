...
const props = defineProps<{ ventas: VentaProps[] }>();
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-col gap-4 overflow-x-auto rounded-xl p-4">
            ...
            <Button>Crear una venta</Button>
            </Link>

            <div>
                <Table>
                    <TableCaption>Lista de todas las ventas.</TableCaption>
                    <TableHeader>
                        <TableRow>
                            <TableHead>
                                ID
                    ...
                    <TableBody>
                        <TableRow v-for="venta in props.ventas" :key="venta.id">
                            <TableCell class="text-sm">{{ venta.id }}</TableCell>
                            <TableCell>{{ venta.cliente_id }}</TableCell>
                            <TableCell>{{ venta.concepto }}</TableCell>
                            <TableCell class="text-center">{{ venta.total }}</TableCell>
                            <TableCell class="text-center gap-2 flex justify-center">
                                ...
                                <Button variant="destructive" size="sm"
                                    @click="handleDelete(venta.id)">Eliminar</Button>
                            </TableCell>
                        </TableRow>
                    </TableBody>
                </Table>
